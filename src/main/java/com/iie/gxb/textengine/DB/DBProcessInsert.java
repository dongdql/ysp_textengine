...
import com.iie.gxb.textengine.main.textEngine;

import jframe.http.HttpUtils;
import net.sf.json.JSONObject;

public class DBProcessInsert {

	final static Log logger = LogFactory.getLog(DBProcessInsert.class.getName());
	...
	public static void process_db(ArrayList<sampleDB> samplesDB, preProcess keywords_white, preProcess keywords_black) {
		...
		for (sampleDB sample : samplesDB) {
			...
			try {
				...
				if (white_result.size() < 1) {
					ArrayList<String> black_result = keywords_match(keywords_black, null, titleStr, descriptionStr);
			...
			if (confirmStr == "true") {
				...
				String sql = "INSERT INTO choujian.gxb_suspicious_result (gxb_suspicious_info_id,gxb_suspicious_type_id,src,state,confirm,confirm_time,engine_callback_time)VALUES ('"
						+ sample.getId() + "','9','KEYWORD','1','0','" + sample.getCreate_time() + "','" + time_f
						+ "')";
				sqls.add(sql);
			}
		...
		if (sqls.size() > 0) {
			Connection connDB = (Connection) DBUtil.getConnection(textEngine.dbUrl);
			InsertResult.insertDB(connDB, sqls);
			try {
				connDB.close();
